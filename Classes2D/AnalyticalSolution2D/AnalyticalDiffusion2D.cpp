...
void AnalyticalDiffusion2D::CalculateAnalyticalValues()
{
    ...
    if (
        TopWallValues.size() != NumOfXNodes ||
    ...
    while (PreviousNumberOfTerms < MaximumNumberOfTerms)
    {
        ...
        if (MaximumDifference < Tolerance && PreviousNumberOfTerms != 0)
        {
            return;
        }
...
) const
{
    ...
    for (std::size_t k = 0; k + 1 < Positions.size(); ++k)
    {
        ...
        const double f0 =
            BoundaryValues[k] *
            std::sin(static_cast<double>(TermNumber) * Pi * x0 / Length);

        const double f1 =
            BoundaryValues[k + 1] *
            std::sin(static_cast<double>(TermNumber) * Pi * x1 / Length);

        Integral += 0.5 * (f0 + f1) * (x1 - x0);
    }

    return 2.0 * Integral / Length;
}
...
) const
{
    ...
    if (DenominatorArgument < 50.0)
    {
        return std::sinh(NumeratorArgument) / std::sinh(DenominatorArgument);
    }
...
void AnalyticalDiffusion2D::AddFourierTerm(std::size_t TermNumber)
{
    ...
    for (std::size_t j = 1; j + 1 < NumOfYNodes; ++j)
    {
        const double y = Height - YPositions[j];
        ...
        for (std::size_t i = 1; i + 1 < NumOfXNodes; ++i)
        {
...
void AnalyticalDiffusion2D::ApplyBoundaryValues()
{
    for (std::size_t i = 0; i < NumOfXNodes; ++i)
    {
        AnalyticalValues[0][i] = TopWallValues[i];
        AnalyticalValues[NumOfYNodes - 1][i] = BottomWallValues[i];
    }
...
void AnalyticalDiffusion2D::PrintAnalyticalValues()
{
    for (std::size_t j = 0; j < NumOfXNodes; j++) 
    {
        for (std::size_t i = 0; i < NumOfYNodes; i++) 
        {
